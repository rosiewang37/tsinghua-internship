...
# Load triples
input_path = r"C:\Tsinghua University Internship\assess_triplets\sample_relation.json"
...
def build_validation_prompt(field, subj, rel, obj, context=""):
    return f"""
你不具备任何事实知识，只判断语言结构是否成立。请判断下列三元组中，主语、谓语(可以表示属性或关系)、宾语是否明确，语义是否通顺。不要因为谓语不合适而标为false.若有指代不明、语义不完整，请标为 false。请判断并给出简洁的理由。
Triple: "{subj}" - "{rel}" - "{obj}"
...
output_path = r"C:\Tsinghua University Internship\assess_triplets\triples_validation_results.jsonl"
false_output_path = r"C:\Tsinghua University Internship\assess_triplets\false_triplets.json"
...
# Save to JSON file
valid_output_path = r"C:\Tsinghua University Internship\assess_triplets\valid_triplets.json"
